...
- `http`: Fetch web content and convert to text, markdown, or HTML formats
  ...
  - No configuration options required
- `artifactory`: Manage JFrog Artifactory repositories and users
  - **Default URL**: `http://localhost`
  - **Default Credentials**: `admin`/`B@55w0rd`
  - **Tools**:
    - `artifactory_healthcheck`: Check Artifactory instance health status
    - `artifactory_get_repositories`: Get list of all repositories
  ...
  - **Configuration Options**:
    ...
    - `username`: Authentication username (defaults to `admin`)
    - `password`: Authentication password (defaults to `B@55w0rd`)
    - `api_key`: Alternative API key authentication
    - `timeout`: Request timeout in seconds (max 120, defaults to 30)
...
The Artifactory builtin server provides comprehensive management of JFrog Artifactory instances. Here are detailed configuration examples:
...
1. **Health Check** (`artifactory_healthcheck`)
   - **Endpoint**: `/router/api/v1/system/health`
   - **Purpose**: Verify Artifactory instance is healthy and operational
   - **Example Usage**:
     ```bash
     mcphost -p "Check the health of my local Artifactory instance"
     ```

2. **Repository Management** (`artifactory_get_repositories`)
   - **Endpoint**: `/artifactory/api/repositories`
   - **Purpose**: List all repositories with their types, descriptions, and URLs
   - **Example Usage**:
     ```bash
     mcphost -p "Get the list of repositories from my local Artifactory instance"
     ```

3. **User Management** (`artifactory_get_users`)
   - **Endpoint**: `/artifactory/api/security/users`
   - **Purpose**: List all users with their roles, realms, and group memberships
   - **Example Usage**:
     ```bash
     mcphost -p "Get the list of users from my local Artifactory instance"
     ```

4. **User Creation** (`artifactory_create_user`)
   - **Endpoint**: `/artifactory/api/security/users/{username}`
   - **Method**: `PUT`
   - **Purpose**: Create new users with configurable permissions and group memberships
   - **Required Parameters**: `new_username`, `new_password`
   - **Optional Parameters**: `email`, `admin`, `realm`, `groups`
   - **Example Usage**:
     ```bash
     mcphost -p "Create a new user called 'developer' with password 'devpass123' and email 'developer@company.com' in my local Artifactory instance"
     ```

5. **Repository Sizes** (`artifactory_get_repository_sizes`)
   - **Endpoints**: 
     - `/artifactory/api/repositories` (get repository list)
     - `/artifactory/api/storage/{repo}` (get individual repository storage info)
   - **Purpose**: Get comprehensive size information for all repositories including file counts, folder counts, and formatted sizes
   - **Example Usage**:
     ```bash
...
6. **Permission Group Management** (`artifactory_manage_permission_group`)
   ...
   - **Purpose**: Create or update permission groups with users, permissions, and repository access in a single operation
   - **Required Parameters**: `group_name`
   - **Optional Parameters**: `group_description`, `users`, `repositories`, `privileges`, `auto_join`, `admin_privileges`, `realm`
   - **Example Usage**:
     ```bash
     # Create a basic group
     mcphost -p "Create a permission group called 'developers' with description 'Development team'"
     
     # Create group with users and repository access
     mcphost -p "Create a permission group called 'qa-team' with users 'tester1,tester2' and grant READ,WRITE permissions to repositories 'test-repo-local,qa-repo-local'"
     
     # Create admin group
     mcphost -p "Create a permission group called 'admins' with admin privileges and auto-join enabled"
     ```

7. **Repository Creation** (`artifactory_create_repository`)
   - **Endpoints**: 
     - `/artifactory/api/repositories/{repoKey}` (create repository)
   - **Purpose**: Create new repositories with support for LOCAL, REMOTE, and VIRTUAL types
   - **Required Parameters**: `repo_key`, `repo_type`, `package_type`
   - **Optional Parameters**: `description`, `notes`, `handle_releases`, `handle_snapshots`, `blacked_out`, `archive_browsing_enabled`
   - **Repository Types**:
     - **LOCAL**: Local repositories for storing artifacts
     - **REMOTE**: Remote repositories for proxying external repositories (requires `remote_url`)
     - **VIRTUAL**: Virtual repositories that aggregate multiple repositories (requires `virtual_repositories`)
   - **Package Types**: Generic, Maven, Gradle, Ivy, Sbt, NuGet, Gems, Npm, Bower, Debian, Composer, PyPI, Docker, GitLfs, YUM, Conan, Chef, Puppet, Helm, Go, P2, R, Swift, CocoaPods, Opkg, Vagrant, Cran, Conda, P2, VCS, etc.
   - **Example Usage**:
     ```bash
     # Create a LOCAL repository
     mcphost -p "Create a LOCAL repository called 'my-local-repo' with package type 'Generic' and description 'My local repository'"
     
     # Create a REMOTE repository
     mcphost -p "Create a REMOTE repository called 'maven-central' with package type 'Maven', remote URL 'https://repo1.maven.org/maven2/', and description 'Maven Central proxy'"
     
     # Create a VIRTUAL repository
     mcphost -p "Create a VIRTUAL repository called 'all-maven' with package type 'Maven', include repositories 'maven-local,maven-central', and description 'All Maven repositories'"
     ```

**Authentication Methods:**
- **Username/Password**: Default method using Basic Authentication
- **API Key**: Alternative method using `X-JFrog-Art-Api` header
- **Environment Variables**: Can be configured using `${env://VAR}` syntax
...
```json
{
  "mcpServers": {
    "artifactory": {
      ...
      "name": "artifactory",
      "options": {
        "base_url": "${env://ARTIFACTORY_URL:-http://localhost}",
        "username": "${env://ARTIFACTORY_USER:-admin}",
        "password": "${env://ARTIFACTORY_PASS:-B@55w0rd}",
        "timeout": "${env://ARTIFACTORY_TIMEOUT:-30}"
      }
    }
  }
}
...
**Common Use Cases:**
- **CI/CD Integration**: Automate repository creation and user management
- **Monitoring**: Regular health checks and storage monitoring
- **Administration**: Bulk user creation and permission management
- **Reporting**: Generate storage usage reports and repository analytics
